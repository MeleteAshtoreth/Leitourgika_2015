...
void runpipe(int pfd[], char *command1, char *command2) 
{ 
	int pid; 
	switch (pid = fork()) 
	{ 
		case 0: /* child */ 
			...
			// close(pipefd[1]);
			char **input2 = malloc(INPUT_SIZE * sizeof(char*));
			input2 = tokenize(command2);			
			if (execvp(input2[0], input2) == -1) 
			{
				perror("Error");
			}
		...
		default: /* parent */ 
			...
			// close(pipefd[0]);
		    char **input1 = malloc(INPUT_SIZE * sizeof(char*));			
			input1 = tokenize(command1);	
			if (execvp(input1[0], input1) == -1) 
			{
				perror("Error");
			}
		case -1: 
...
int execPipe(char *command)
{
	...
	{
			// runpipe(pipefd, command1, command2);
			dup2(pipefd[0], 0); 
			close(pipefd[0]);  
			char **input2 = malloc(INPUT_SIZE * sizeof(char*));
			input2 = tokenize(command2);			
